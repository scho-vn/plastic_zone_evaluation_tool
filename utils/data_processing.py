...
class Data_Processing:
    def __init__(self, specimen_name: str = "not defined", side: str = None,
                 nodemap_name: str = None, specimen_type :str = None ):
        ...
        for folder in [self.output_path_image,self.output_path_contours ,self.output_path_nodemaps,
                       self.output_path_results, self.output_path_pickle]:
    ...
    def mask_data(self, crack_tip_x: float = None, crack_tip_y : float = None,
                  ...
                  strain_treshold : float = 0.68, crack_tip_tolerance : float = 0.1,
                  reduce_x_window: tuple = (0,0), reduce_y_window : tuple = (0,0)):
        ...
        if self.specimen_type == 'FE':
            x_int = np.arange(start=x_coordinates.min(), stop=x_coordinates.max(), step=0.02)
            y_int = np.arange(start=y_coordinates.min(), stop=y_coordinates.max(), step=0.02)


        else:
            x_int = np.arange(start=x_coordinates.min(), stop=x_coordinates.max(), step=0.01)
            y_int = np.arange(start=y_coordinates.min(), stop=y_coordinates.max(), step=0.01)
        xi, yi = np.meshgrid(x_int, y_int)
        zi = griddata((x_coordinates, y_coordinates), strains, (xi, yi), method='linear')
        ...
        x_window = (in_front_of_ct_x, (xi < x_coordinates.max() - self.reduce_x_window[1]))

        y_window = ((yi < self.crack_tip_y + self.reduce_y_window[0]), (yi > self.crack_tip_y - self.reduce_y_window[1]))
        ...
        if self.specimen_type == 'Biax':
            mask_all = 1* (np.all([thresholded_strains, x_window[0], x_window[1],y_window[0],y_window[1] ], axis=0))
            mask_lower= 1*(np.all([thresholded_strains,  x_window[0], x_window[1],upper_half, y_window[1]], axis=0))
            mask_upper = 1*(np.all([thresholded_strains,  x_window[0], x_window[1],lower_half, y_window[0]], axis=0))
        if self.specimen_type == 'MT':
            mask_all = 1* (np.all([thresholded_strains,  x_window[0], x_window[1]], axis=0))
            mask_lower= 1*(np.all([thresholded_strains,  x_window[0], x_window[1],upper_half], axis=0))
            mask_upper = 1*(np.all([thresholded_strains, x_window[0], x_window[1],lower_half], axis=0))
        ...
        for key, value in masked_dict.items():
            ...
            norm_image = cv2.normalize(value, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8UC1)
            contours, hierarchy = cv2.findContours(norm_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
            self.key_to_contour.update({f'{key}' :{'Contour': contours, 'Hierarchy': hierarchy}})
    ...
    def evaluate_contours(self, which_contours : list =['Whole'], secondary_crack_treshold: float = 80):
        """
        ...
        x_int, y_int, = griddata[0][0], griddata[1][:,0].flatten()
        ...
        for item in self.list_of_contours:
            ...
            if count_found_contours == 0:
                ...
                if count_found_contours > 1:
                    ...
                    contour_all = sorted(key_to_contour[item]['Contour'], key=cv2.contourArea,
                                 reverse=True)  # cnt[0] largest cnt[1] second largest
                    contour_to_analyze = max(key_to_contour[item]['Contour'], key=cv2.contourArea)
                ...
                extLeft = tuple(contour_to_analyze[contour_to_analyze[:, :, 0].argmin()][0])
                x_left, y_left = x_int[int(extLeft[0])], y_int[int(extLeft[1])]
                extRight = tuple(contour_to_analyze[contour_to_analyze[:, :, 0].argmax()][0])
                x_right, y_right = x_int[int(extRight[0])], y_int[int(extRight[1])]
                extTop = tuple(contour_to_analyze[contour_to_analyze[:, :, 1].argmin()][0])
                x_top, y_top = x_int[int(extTop[0])], y_int[int(extTop[1])]
                extBot = tuple(contour_to_analyze[contour_to_analyze[:, :, 1].argmax()][0])
                ...
                cog_x, cog_y = x_int[int(moments['m10'] / moments['m00'])], y_int[int(moments['m01'] / moments['m00'])]
                ...
                contour_lenght_pz = (cv2.arcLength(contour_to_analyze, closed=True)) / pixelsize

                #calculate angle between crack tip and center of gravity in degree
                angle_ct_to_cog = math.degrees(math.atan2(cog_y-self.crack_tip_y,cog_x-self.crack_tip_x))
                ...
                if self.specimen_type =='Biax':
                    self.key_to_results.update({f'{item}': {'Cycles': self.cycles, 'Filename': self.nodemap_name,
                                                           'Crack Tip X[mm]': self.crack_tip_x,'Crack Tip Y[mm]': self.crack_tip_y,
                                                           'Crack lenght[mm]': self.nodemap_file.cracklength,'Area PZ[mm²]': area_pz,
                                                           'COG_X[mm]': cog_x,'COG_Y[mm]': cog_y, 'Contour lenght[mm]':contour_lenght_pz,
                                                           'Angle[°]': angle_ct_to_cog,'Epsmax[%]': self.nodemap_file.eps_vm.max(),
                                                           'Threshold': self.strain_treshold,'X_min[mm]': self.nodemap_file.coor_x.min(),
                                                           'X_max[mm]':self.nodemap_file.coor_x.max(),'Y_min[mm]': self.nodemap_file.coor_y.min(),
                                                           'Y_max[mm]': self.nodemap_file.coor_y.max(), 'Pixelsize': pixelsize,
                                                           'Height': pz_height, 'Lenght': contour_lenght_pz, 'Secondary crack': sec_crack,
                                                            'Largest Contour [px]': contour_to_analyze,
                                                            'Ext_Bottom': (x_bottom, y_bottom), 'Ext_Top':(x_top, y_top),
                                                            'Ext_Left': (x_left, y_left), 'Ext_Right':(x_right, y_right),
                                                            'Largest contour [mm]':(pixel_to_x_coords,pixel_to_y_coords)}})
                    self.nodemap_to_results.update({f'{self.nodemap_name}': self.key_to_results})
                ...
                if self.specimen_type =='MT':
                    self.key_to_results.update({f'{item}': {'Cycles': self.cycles, 'Filename': f'{self.nodemap_folder_id}_{self.nodemap_name}',
                                                           'Crack Tip X[mm]': self.crack_tip_x,'Crack Tip Y[mm]': self.crack_tip_y,
                                                           'Crack lenght[mm]': self.nodemap_file.cracklength,'Area PZ[mm²]': area_pz,
                                                           'COG_X[mm]': cog_x,'COG_Y[mm]': cog_y, 'Contour lenght[mm]':contour_lenght_pz,
                                                           'Angle[°]': angle_ct_to_cog,'Epsmax[%]': self.nodemap_file.eps_vm.max(),
                                                           ...
                                                           'Height': pz_height, 'Lenght': contour_lenght_pz, 'Secondary crack': sec_crack,
                                                            ...
                                                            'Ext_Left': (x_left, y_left), 'Ext_Right':(x_right, y_right),
                                                            'Largest contour [mm]':(pixel_to_x_coords,pixel_to_y_coords)}})
                    self.nodemap_to_results.update({f'{self.nodemap_folder_id}_{self.nodemap_name}': self.key_to_results})


                if self.specimen_type =='FE':
                    self.key_to_results.update({f'{item}': {'Cycles': self.cycles, 'Filename': f'{self.nodemap_name}',
                                                           'Crack Tip X[mm]': self.crack_tip_x,'Crack Tip Y[mm]': self.crack_tip_y,
                                                           'Crack lenght[mm]': self.nodemap_file.cracklength,'Area PZ[mm²]': area_pz,
                                                           'COG_X[mm]': cog_x,'COG_Y[mm]': cog_y, 'Contour lenght[mm]':contour_lenght_pz,
                                                           'Angle[°]': angle_ct_to_cog,'Epsmax[%]': self.nodemap_file.eps_vm.max(),
