...
def sum_results(specimen_name: str = "not defined", side: str = None, result_path: str = None,
                delete_single_files: bool = True, key_index : str = None):
    ...
    df_concat = pd.concat([pd.read_csv(os.path.join(result_path,f)) for f in sorted(csv_files) ], ignore_index=False).set_index(key_index, drop=False)
...
def pickle_output(specimen_name: str = "not defined", side: str = None, result_path: str = None,
                which: str = None):
    """
    ...
    with open(os.path.join(result_path,f'{specimen_name}_{side}_Plastic_Zone.pickle'), 'wb') as handle:
        pickle.dump(which, handle, protocol=pickle.HIGHEST_PROTOCOL)


def json_output(specimen_name: str = "not defined", side: str = None, result_path: str = None,
                which: str = None):
    ...
    with open(os.path.join(result_path,f'{specimen_name}_{side}_Plastic_Zone.json'), 'w') as handle:
        print('done')
